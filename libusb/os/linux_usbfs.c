...
static int android_jni_add_device_descriptors(JNIEnv *jni_env, struct linux_device_priv *priv)
{
	...
	struct usbi_device_descriptor device_desc = {
		...
		.bDeviceSubClass = (*jni_env)->CallIntMethod(jni_env, device,
			(*jni_env)->GetMethodID(
				jni_env,
				UsbDevice,
				"getDeviceSubclass",
		...
		.bDeviceProtocol = (*jni_env)->CallIntMethod(jni_env, device,
			(*jni_env)->GetMethodID(
				jni_env,
				UsbDevice,
				"getDeviceProtocol",
...
static int android_jni_initialize_device(struct libusb_device *dev, uint8_t busnum, uint8_t devaddr)
{
	...
	while (android_jni_devices_next(devices, &iter_device, &iter_busnum, &iter_devaddr) == LIBUSB_SUCCESS) {
